...
static unsigned int changed_light = 0;
...
static void notify(int light_id)
{
	...
	OCRepPayloadSetUri(payload, ocf_mylight_light_peek_uri(light_id));

	ocf_mylight_light_get_status(light_id, &value);
	OCRepPayloadSetPropBool(payload, "value", value);

	ocf_mylight_light_get_handle(light_id, &handle);
